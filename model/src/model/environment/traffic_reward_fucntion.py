...
class TrafficRewardFunction:
    ...
    def __init__(self, 
                 ...
                 waiting_weight: float = 0.4, 
                 throughput_weight: float = 0.2, 
                 penalty_weight: float = 0.1,
                 soft_queue_penalty_weight: float = 0.01):
        """
        Args:
            ...
            throughput_weight: Weight for vehicle throughput reward
    ...
    def calculate_reward(self, 
                         ...
                         action: int, 
                         throughput: int,
                         recent_phase_switches: int = 0) -> float:
        ...
        if prev_state['current_phase'] != current_state['current_phase']:
            ...
            if duration < 5:
                ...
                penalty = 0
            phase_change_penalty = penalty * self.penalty_weight

        if recent_phase_switches > 3:
            phase_change_penalty += -self.penalty_weight * (recent_phase_switches - 3)

        residual_queue_penalty = -self.soft_queue_penalty_weight * curr_total_queue

        total_reward = (queue_reward + waiting_reward + throughput_reward + 
