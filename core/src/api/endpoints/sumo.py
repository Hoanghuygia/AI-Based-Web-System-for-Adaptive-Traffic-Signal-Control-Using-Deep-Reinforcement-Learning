...
@router.post("/traffic-light/{light_id}/phase/{phase}", tags=["sumo"])
async def change_traffic_light_phase(light_id: str, phase: int):
    print(f"Changing traffic light {light_id} to phase {phase}")
...
@router.get("/traffic-light/{light_id}/phase", tags=["sumo"])
async def get_traffic_light_phase_api(light_id: str):
    print(f"Fetching current phase for traffic light {light_id}")
